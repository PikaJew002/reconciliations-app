...
let runForm = useForm({});
let isRunInProgress = computed(() => ['pending', 'processing'].includes(props.activeRun?.status));
let activeTab = ref((props.summary.needs_review ?? 0) > 0 ? 'needs-review' : 'matched');
...
function isWalmartTransaction(transaction) {
    ...
    return merchant.includes('walmart')
...
function isTransferTransaction(transaction) {
    return (transaction.description || '').toLowerCase().includes('transfer');
}
...
let unmatchedTransactionFilters = computed(() => {
    ...
    return [
        ...
        { id: 'untagged-transfer', label: 'Untagged (transfer)', count: counts['untagged-transfer'] },
        { id: 'untagged-other', label: 'Untagged (Other)', count: counts['untagged-other'] },
    ];
});
...
let filteredUnmatchedTransactions = computed(() => {
    ...
    return props.unmatchedTransactions.filter((transaction) => (
        unmatchedTransactionCategory(transaction) === unmatchedTransactionFilter.value
    ));
});
...
function deleteComponent(order, component) {
    ...
    router.delete(`/reconciliation/orders/${order.id}/components/${component.id}`, {
        preserveScroll: true,
    });
}
...
function selectMatchingBankTransaction(order, paymentIndex, amount) {
    ...
    if (row.bank_transaction_id) {
        let selected = payment.candidate_transactions.find((tx) => tx.id === Number(row.bank_transaction_id));

        if (!selected || !amountsMatch(Math.abs(Number(selected.amount)), amount)) {
...
function onPaymentTransactionSelected(order, paymentIndex) {
    ...
    let selected = payment.candidate_transactions.find((tx) => tx.id === selectedId);
    ...
    if (otherIndexes.length === 1) {
        let otherIndex = otherIndexes[0];
        let remainder = Number(order.total) - Math.abs(Number(selected.amount));
        rows[otherIndex].amount = remainder.toFixed(2);
    }
}
...
function resolvePayments(order) {
    ...
    router.post(`/reconciliation/orders/${order.id}/resolve-payments`, {
        payments: rows.map((row) => ({
            index: row.index,
            amount: Number(row.amount),
            bank_transaction_id: row.bank_transaction_id ? Number(row.bank_transaction_id) : null,
        })),
    }, {
        preserveScroll: true,
        onSuccess: () => {
...
<template>
    <div class="mx-auto max-w-3xl space-y-8 p-8">
        <div class="flex items-center justify-between gap-4">
            ...
            >
                {{ isRunInProgress || runForm.processing ? 'Running…' : 'Run reconciliation' }}
            </button>
        </div>
        ...
        <p v-if="flashError" class="rounded border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-800">
            {{ flashError }}
        </p>
        ...
        >
            Reconciliation {{ activeRun.status }}… this page updates automatically.
        </p>
        ...
        >
            ...
            Matched {{ activeRun.metadata?.merchants_matched ?? 0 }} merchants,
            {{ activeRun.metadata?.transactions_matched ?? 0 }} order transactions,
            and {{ activeRun.metadata?.synthetic_matched ?? 0 }} synthetic bank spends.
        </p>
        ...
        >
            Reconciliation failed{{ activeRun.error_message ? `: ${activeRun.error_message}` : '.' }}
        </p>

        <p class="text-sm text-neutral-600">
            Re-runs merchant matching, Walmart order matching, and synthetic bank-spend reconciliation
            on data you have already imported.
        </p>

        <dl class="grid grid-cols-2 gap-3 text-sm sm:grid-cols-3">
            <div class="rounded border px-3 py-2">
                <dt class="text-neutral-600">Needs review</dt>
                <dd class="text-lg font-medium">{{ summary.needs_review ?? 0 }}</dd>
            </div>
            <div class="rounded border px-3 py-2">
        ...
        <div class="space-y-4">
            ...
            <section v-if="activeTab === 'needs-review'" class="space-y-8">
                <div class="space-y-4">
                    <div>
                        ...
                        <p class="text-sm text-neutral-600">
                            Orders paid with more than one method (card + gift card / Walmart Balance).
                            Match the bank card charge, enter the other tender amount, then save.
                        </p>
                    </div>

                    <p v-if="paymentReviewOrders.length === 0" class="text-sm text-neutral-600">
                        ...
                        >
                            <div class="flex items-start justify-between gap-4">
                                <div>
                                    ...
                                    <p class="text-neutral-600">{{ order.ordered_at || 'No date' }}</p>
                                </div>
                            ...
                            >
                                Fix unbalanced components below first, then resolve payments.
                            </p>

                            <form
                                v-if="paymentForms[order.id] && order.components_balanced"
                                class="space-y-3"
                            ...
                            >
                                ...
                                >
                                    <div class="flex items-center justify-between gap-3">
                                        <div>
                                            <p class="font-medium">{{ payment.ending }}</p>
                                            <p class="text-neutral-600">{{ paymentKindLabel(payment.kind) }}</p>
                                        </div>
                                    </div>
                                    ...
                                    <div class="grid gap-3 sm:grid-cols-2">
                                        <label class="block space-y-1">
                                            ...
                                            <input
                                                v-model="paymentForms[order.id][paymentIndex].amount"
                                                type="number"
                                                step="0.01"
                                                min="0.01"
                                                class="w-full rounded border px-2 py-1.5"
                                                required
                                                @blur="onPaymentAmountBlur(order, paymentIndex)"
                                            >
                                        </label>

                                        <label
                                            v-if="payment.requires_bank_transaction"
                                            class="block space-y-1"
                                        >
                                            <span class="text-neutral-600">Bank transaction</span>
                                            <select
                                                v-model="paymentForms[order.id][paymentIndex].bank_transaction_id"
                                                class="w-full rounded border px-2 py-1.5"
                                                required
                                                @change="onPaymentTransactionSelected(order, paymentIndex)"
                                            >
                                                ...
                                                >
                                                    ...
                                                    · {{ tx.transaction_date || tx.posted_at || 'No date' }}
                                                    <template v-if="tx.card_last_four"> · card {{ tx.card_last_four }}</template>
                                                </option>
                                            </select>
                                        </label>
                                ...
                                >
                                    {{ resolvingOrderId === order.id ? 'Saving…' : 'Resolve payments' }}
                                </button>
                            </form>
                        </li>
                    </ul>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <p class="text-sm text-neutral-600">
                            Orders whose components do not add up to the order total. Add the missing fee
                            (for example Fast delivery) or remove a bad component, then re-run reconciliation.
                        </p>
                    </div>

                <p v-if="unbalancedOrders.length === 0" class="text-sm text-neutral-600">
                    ...
                    >
                        <div class="flex items-start justify-between gap-4">
                            ...
                            <div class="text-right">
                                <p class="font-medium">{{ formatMoney(order.total) }} total</p>
                                <p class="text-neutral-600">
                                    Components {{ formatMoney(order.component_sum) }}
                                </p>
                        ...
                        <ul class="divide-y rounded border">
                            ...
                            >
                                <div>
                                    ...
                                    <p class="text-neutral-600">
                                        {{ component.type }}
                                        <span v-if="component.is_user_modified"> · manual</span>
                                    </p>
                                </div>
                                <div class="flex items-center gap-3">
                                    <p class="font-medium">{{ formatMoney(component.amount) }}</p>
                                    <button
                        ...
                        >
                            ...
                            <div class="sm:col-span-4">
                                ...
                                >
                                    {{ savingOrderId === order.id ? 'Saving…' : 'Add component' }}
                                </button>
                            </div>
                        </form>
                    </li>
            ...
            <section v-else-if="activeTab === 'unmatched-orders'" class="space-y-3">
            ...
            <section v-else class="space-y-3">
                ...
                >
                    <button
                        ...
                        type="button"
                        class="rounded px-3 py-1.5 text-sm"
                        :class="unmatchedTransactionFilter === filter.id
                            ? 'bg-neutral-900 text-white'
                ...
                <p v-if="unmatchedTransactions.length === 0" class="text-sm text-neutral-600">
                    No unmatched transactions.
                </p>

                <p v-else-if="filteredUnmatchedTransactions.length === 0" class="text-sm text-neutral-600">
                ...
                <ul v-else class="divide-y rounded border">
                    ...
                    >
                        <div>
                            ...
                            <p class="text-neutral-600">
                                {{ transaction.transaction_date || transaction.posted_at || 'No date' }}
                        ...
                        <div class="text-right">
                            ...
                            <p class="text-neutral-600">{{ transaction.status }}</p>
                        </div>
                    </li>
                </ul>
            </section>
